...
object GlyphCache {
    private val directory = "${NanoSenseMod.DIRECTORY}/glyphs"
    ...
    fun put(font: Font, chunk: Int, image: BufferedImage, charInfoArray: Array<CharInfo>) {
        ...
        runCatching {
            ...
            NanoSenseMod.logger.info("Failed saving glyph cache", e)
            runCatching {
                imageFile.delete()
                infoFile.delete()
            }.onFailure {
                NanoSenseMod.logger.error("Error saving glyph cache", it)
            }
        }
    }
    ...
    fun get(font: Font, chunk: Int): Pair<BufferedImage, Array<CharInfo>>? {
        return runCatching {
            ...
            NanoSenseMod.logger.error("Error reading glyph cache", it)
        }.getOrNull()
    }
