package dev.supermic.nanosense.gui.rgui
...
import dev.supermic.nanosense.util.delegate.FrameValue
import dev.supermic.nanosense.util.graphics.HAlign
import dev.supermic.nanosense.util.graphics.RenderUtils3D
import dev.supermic.nanosense.util.graphics.VAlign
...
open class Component(
    ...
    protected var relativePosX by setting("Pos X", posXIn, -69420.911f..69420.911f, 0.1f, { false },
        { _, it -> if (this is WindowComponent && NanoSenseMod.ready) absToRelativeX(relativeToAbsX(it).coerceIn(0.0f, max(scaledDisplayWidth - width, 0.0f))) else it })
    protected var relativePosY by setting("Pos Y", posYIn, -69420.911f..69420.911f, 0.1f, { false },
        { _, it -> if (this is WindowComponent && NanoSenseMod.ready) absToRelativeY(relativeToAbsY(it).coerceIn(0.0f, max(scaledDisplayHeight - height, 0.0f))) else it })
