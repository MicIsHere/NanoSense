...
import dev.supermic.nanosense.gui.hudgui.elements.client.Watermark
import dev.supermic.nanosense.manager.managers.*
import dev.supermic.nanosense.manager.managers.HotbarManager.serverSideItem
import dev.supermic.nanosense.manager.managers.HotbarManager.spoofHotbar
import dev.supermic.nanosense.manager.managers.HotbarManager.spoofHotbarPlus
...
import dev.supermic.nanosense.util.EntityUtils.isPassive
...
import dev.supermic.nanosense.util.accessor.*
import dev.supermic.nanosense.util.collections.CircularArray
import dev.supermic.nanosense.util.combat.CombatUtils.scaledHealth
import dev.supermic.nanosense.util.combat.CombatUtils.totalHealth
import dev.supermic.nanosense.util.combat.CrystalDamage
import dev.supermic.nanosense.util.combat.CrystalUtils
import dev.supermic.nanosense.util.combat.CrystalUtils.canPlaceCrystalOn
import dev.supermic.nanosense.util.combat.CrystalUtils.isResistant
import dev.supermic.nanosense.util.combat.ExposureSample
...
import dev.supermic.nanosense.util.graphics.*
...
import dev.supermic.nanosense.util.items.attackDamage
import dev.supermic.nanosense.util.items.duraPercentage
import dev.supermic.nanosense.util.math.RotationUtils
import dev.supermic.nanosense.util.math.RotationUtils.getRotationTo
import dev.supermic.nanosense.util.math.VectorUtils.setAndAdd
import dev.supermic.nanosense.util.math.VectorUtils.toViewVec
import dev.supermic.nanosense.util.math.vector.*
import dev.supermic.nanosense.util.pause.HandPause
...
import dev.supermic.nanosense.util.threads.*
...
import dev.supermic.nanosense.util.world.rayTraceVisible
...
internal object SmartCrystal : Module(
    name = "SmartCrystal",
    description = "Lol wtf",
    ...
    init {
        onEnable {
            ...
            NanoSenseMod.logger.trace(Watermark.nameAsString)
        }
    ...
    private inline fun SafeClientEvent.placeDirect(placeInfo: PlaceInfo) {
        ...
        if (hand == null) {
            when (autoSwap) {
                ...
                SwapMode.SWAP -> {
                    ...
                    onMainThread {
                        ...
                        MainHandPause.withPause(SmartCrystal, placeDelay * 2) {
                            swapToSlot(slot)
                        ...
                        try{
                            if(mc.isCallingFromMinecraftThread){
                                synchronized(mc.playerController){
                                    Minecraft.getMinecraft().player.inventory.currentItem = slot.hotbarSlot
                                    Minecraft.getMinecraft().playerController.updateController()
                                }
                            }
                        }catch(e: Exception){
                            MessageSendUtils.sendNoSpamChatMessage("HotbarManager.SafeClientEvent.spoofHotbarPlus.Exception: $e")
                        }
                    }
                }
            }
            ...
            onMainThread {
                HandPause[hand].withPause(SmartCrystal, placeDelay * 2) {
