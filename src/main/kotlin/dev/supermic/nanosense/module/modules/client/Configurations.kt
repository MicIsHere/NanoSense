...
import dev.supermic.nanosense.setting.ConfigManager
...
import dev.supermic.nanosense.setting.settings.impl.primitive.StringSetting
...
internal object Configurations : AbstractModule(
    ...
    init {
        BackgroundScope.launchLooping("Config Auto Saving", 60000L) {
            if (autoSaving && mc.currentScreen !is AbstractNanoGui<*, *> && timer.tickAndReset(savingInterval.toLong())) {
                if (savingFeedBack) NoSpamMessage.sendMessage(this@Configurations, "Auto saving settings...")
                else NanoSenseMod.logger.info("Auto saving settings...")
                ConfigUtils.saveAll()
            }
        }
    ...
    private fun updatePreset(setting: StringSetting, input: String, config: IConfig) {
        ...
        try {
            ...
            NanoSenseMod.logger.warn("Couldn't set path!", e)
        ...
        fun copyPreset(name: String) {
            defaultScope.launch(Dispatchers.IO) {
                ...
                try {
                    ...
                    NanoSenseMod.logger.error("Failed to copy preset", e)
                }
            }
        }
        ...
        fun deletePreset(name: String) {
            defaultScope.launch(Dispatchers.IO) {
                ...
                try {
                    ...
                    NanoSenseMod.logger.error("Failed to delete preset", e)
                }
            }
        }
