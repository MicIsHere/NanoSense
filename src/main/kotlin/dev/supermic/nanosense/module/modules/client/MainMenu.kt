...
import dev.supermic.nanosense.module.modules.render.AntiAlias
...
import dev.supermic.nanosense.util.extension.mapEach
import dev.supermic.nanosense.util.extension.normalizeCase
...
import dev.supermic.nanosense.util.graphics.font.renderer.FontRenderer
import dev.supermic.nanosense.util.graphics.font.renderer.MainFontRenderer
import dev.supermic.nanosense.util.graphics.shaders.GLSLSandbox
import dev.supermic.nanosense.util.interfaces.DisplayEnum
import dev.supermic.nanosense.util.math.Box
...
internal object MainMenu : AbstractModule(
    ...
    private val title by setting("Title", Title.NANOSENSE)
    ...
    private enum class ShaderEnum {
        ...
        val path = name
            ...
            .joinToString("", "/assets/nanosense/shaders/menu/", ".fsh")
    }
    ...
    init {
        listener<GuiEvent.Displayed> {
            if (it.screen is GuiMainMenu) {
                it.screen = NanoGuiMainMenu()
            }
        }
        (NanoGuiMainMenu.Companion).toString()
    }
    ...
    class NanoGuiMainMenu : GuiScreen() {
    ...
    private object TitleFontRender : FontRenderer(Font.createFont(Font.TRUETYPE_FONT, this::class.java.getResourceAsStream("/assets/nanosense/fonts/Orbitron-Regular.ttf")), 80.0f, 5120) {
        override val shadowDist: Float
            get() = 4.0f
    }

    private object ButtonFontRenderer : FontRenderer(Font.createFont(Font.TRUETYPE_FONT, this::class.java.getResourceAsStream("/assets/nanosense/fonts/GOTHIC.TTF")), 36.0f, 2048)
}
