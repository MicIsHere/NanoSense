package dev.supermic.nanosense.translation

import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.Job
import kotlinx.coroutines.launch
import dev.supermic.nanosense.NanoSenseMod
import dev.supermic.nanosense.module.modules.client.Language
...
object TranslationManager {
    ...
    fun checkUpdate() {
        defaultScope.launch(Dispatchers.IO) {
            try {
                ...
                NanoSenseMod.logger.warn("Failed to check for translation updates", e)
            }
        }
    }
    ...
    fun reload() {
        ...
        try {
            ...
            translationMap = map
            NanoSenseMod.logger.info("Loaded language $language from $source")
            TranslationKey.updateAll()
        } catch (e: IllegalArgumentException) {
            NanoSenseMod.logger.warn(e.message)
        } catch (e: Exception) {
            NanoSenseMod.logger.warn("Failed to load language $language", e)
        }
    }
