...
import dev.supermic.nanosense.util.ConnectionUtils
...
object UUIDManager : Manager() {
    private val file = File("${NanoSenseMod.DIRECTORY}/uuid_cache.txt")
    ...
    private fun requestProfile(nameOrUUID: String): PlayerProfile? {
        ...
        return if (response.isNullOrBlank()) {
            NanoSenseMod.logger.error("Response is null or blank, internet might be down")
            null
        } else {
            try {
                ...
                NanoSenseMod.logger.error("Failed parsing profile", e)
                null
            }
        }
    }
    ...
    private fun request(url: String): String? {
        return ConnectionUtils.requestRawJsonFrom(url) {
            NanoSenseMod.logger.error("Failed requesting from Mojang API", it)
        }
    }
    ...
    fun load(): Boolean {
        ...
        return try {
            ...
            NanoSenseMod.logger.info("UUID cache loaded")
            true
        } catch (e: Exception) {
            NanoSenseMod.logger.warn("Failed loading UUID cache", e)
            false
        }
    }
    ...
    fun save(): Boolean {
        return try {
            ...
            NanoSenseMod.logger.info("UUID cache saved")
            true
        } catch (e: Exception) {
            NanoSenseMod.logger.warn("Failed saving UUID cache", e)
            false
        }
    }
