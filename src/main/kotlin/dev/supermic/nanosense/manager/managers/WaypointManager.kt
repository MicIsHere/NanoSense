...
import dev.supermic.nanosense.event.events.WaypointUpdateEvent
...
import dev.supermic.nanosense.util.math.CoordinateConverter
...
object WaypointManager : Manager() {
    ...
    private val file = File("${NanoSenseMod.DIRECTORY}/waypoints.json")
    ...
    fun loadWaypoints(): Boolean {
        ...
        val success = try {
            ...
            NanoSenseMod.logger.info("Waypoint loaded")
            true
        } catch (e: Exception) {
            NanoSenseMod.logger.warn("Failed loading waypoints", e)
            false
        }
    ...
    fun saveWaypoints(): Boolean {
        return try {
            ...
            NanoSenseMod.logger.info("Waypoint saved")
            true
        } catch (e: Exception) {
            NanoSenseMod.logger.warn("Failed saving waypoint", e)
            false
        }
    }
    ...
    fun add(locationName: String): Waypoint {
        ...
        return if (pos != null) {
            ...
            NanoSenseMod.logger.error("Error during waypoint adding")
