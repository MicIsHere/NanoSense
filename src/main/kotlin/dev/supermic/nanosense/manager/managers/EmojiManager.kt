...
import dev.supermic.nanosense.util.graphics.texture.MipmapTexture
import dev.supermic.nanosense.util.readText
...
object EmojiManager : Manager() {
    private val localFile = File("${NanoSenseMod.DIRECTORY}/emoji.zip")
    ...
    private val job = defaultScope.launch(Dispatchers.IO) {
        try {
            ...
            NanoSenseMod.logger.warn("Failed to check emoji update", e)
        }

        try {
            zipCache = ZipCache(ZipFile(localFile))
            NanoSenseMod.logger.info("Emoji Initialized")
        } catch (e: Exception) {
            NanoSenseMod.logger.warn("Failed to load emojis", e)
        }
    }
    ...
    private fun streamToJson(stream: InputStream): JsonObject? {
        return try {
            ...
            NanoSenseMod.logger.warn("Failed to parse emoji version Json", e)
            null
        }
    }
    ...
    private fun loadEmoji(name: String) {
        ...
        try {
            ...
            NanoSenseMod.logger.warn("Failed to load emoji", e)
        }
    }
